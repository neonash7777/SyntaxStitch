...
3. In the GitHub repository, add the Actions variable `VSCE_PUBLISHER` containing the public publisher ID.
...
2. Move relevant entries from **Unreleased** to a dated version section in `CHANGELOG.md` and update its comparison links.
3. Run `npm test` and `npm run package:vsix`.
4. Commit and push the release changes.
5. Create and publish a GitHub Release tagged with the exact package version prefixed by `v`, such as `v0.1.0`.
